...
const questions = [
  "I avoid buying single-use products.",
  "I try to repair or reuse items instead of replacing them.",
  "I choose durable, high-quality goods even if they cost more.",
  "I buy second-hand or recycled products when possible.",
];

export default function ConsumptionSurvey() {
  const [answers, setAnswers] = useState<number[]>(Array(questions.length).fill(3));
  const [selected, setSelected] = useState<number>(0);

  // Save responses locally under the 'consumptionCategory' key
  useEffect(() => {
    localStorage.setItem("consumptionCategory", JSON.stringify(answers));
  }, [answers]);
  ...
  // set answer for currently selected question when slider moves
  ...
  return (
    <section className="flex flex-col items-center gap-6">
      ...
      <div className="w-full max-w-2xl flex flex-col items-center gap-4">
        ...
        {/* Shared slider for the selected question */}
        <div className="flex flex-col items-center mt-2">
          ...
          {/* Labels correlated with the moving bar (single shared legend) */}
