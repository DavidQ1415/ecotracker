...
const questions = [
  "I use public transportation regularly.",
  "I walk or bike instead of driving for short distances.",
  "I limit air travel when possible.",
];

export default function TransportSurvey() {
  const [answers, setAnswers] = useState<number[]>(Array(questions.length).fill(3));
  const [selected, setSelected] = useState<number>(0);

  useEffect(() => {
    localStorage.setItem("transportCategory", JSON.stringify(answers));
  }, [answers]);
  ...
  return (
    <section className="flex flex-col items-center gap-6">
      <h2 className="text-2xl font-semibold text-[var(--accent)]">Transport</h2>
